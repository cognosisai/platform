...
const { minGenerate, nlpcloud_generate, nlpcloud_tokens, openai_generate } = proxyActivities<typeof llm>({ startToCloseTimeout: '10 minute', });
const { nlpcloud_tokenize, tokenize_native } = proxyActivities<typeof tokenizer>({ startToCloseTimeout: '10 minute', });
const { es_context } = proxyActivities<typeof elastic>({ startToCloseTimeout: '10 minute', });
const { actionLogger, nlp_stable_diffusion } = proxyActivities<typeof util>({ startToCloseTimeout: '10 minute', });


export async function promptTemplate< T >( template: string, variables: T, minLength: number = 1, maxLength: number = 50, temperature: number = 0.0, model: "gpt-3" | "gpt-neox-20b" = "gpt-neox-20b" ): Promise< string >
{
  let prompt = mustache.render( template, variables );
  let response = await minGenerate( prompt, minLength, maxLength, temperature, model );
  return response;
}
 ...
 * @returns 
 */
 export async function splitPromptTemplateByTokens( data: string, template: string, minLength: number = 1, maxLength: number = 50, temperature: number = 0.0 ): Promise< Array<[string, string]> >
 {
   let template_token_length = await tokenize_native( template ).then( (r) => r.length );
   ...
   let token_split_promises = token_split.map( async (t) => {
     let p = await promptTemplate( template, {chunk: t.join(" ")}, minLength, maxLength, temperature );
     return [t.join(" "), p];
   });
 ...
 */
   ...
   let allchunks = chunks.map( async (chunk, i, a): Promise< [string, string, number[]] > => {
 ...
 function splitUp< T >(arr: T[], size: number ): T[][] {
  ...
  export async function promptReducer( inPrompt: string, variables: any, preamble: string, instructions: string ): Promise< string >
  {
  ...
  export async function translateQuerySpaceToAnswerSpace( query: string ): Promise< string >
  {
    let p =
  `Translate from "query space" to "answer space" while preserving the unknowns as variables.
  ...
  {
    let p =
  `We are going to extract full-text search queries for the following query:
  ...
  export async function questionAndAnswer( index: string, query: string ): Promise< QandA >
  {
    ...
    let results = await embeddingsFromTextSearch( index, await keywordsFromQuery(query), 5 );
  ...
  { "answer": "`;
    let result = `{ "answer": "` + await promptTemplate( prompt, {query: query, context: context.join("\n")}, 2, 250, 0.0 );
    try
    {
...
async function testAnswerFromQuestion( question: string, answer: string ): Promise< boolean >
{
  ...
  Is the answer correct? yes/no:`;
    let result = await promptTemplate( p, {question: question, answer: answer}, 2, 5, 0.0 );
...
let summaries = await splitPromptTemplateByLinesOfTokens( text, primarySummarizeTemplate, 5, 1024, 0.2 );
...
while( true )
{
  ...
  let summary = await splitPromptTemplateByLinesOfTokens( completions.join("\n\n"), reduceTemplate, 500, 1000, 0.2 );
