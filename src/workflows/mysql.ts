...
const { dbquery } = proxyActivities<typeof mysql>({ startToCloseTimeout: '10 minute', });
const { promptReducer, promptTemplate, splitPromptTemplateByLinesOfTokens, splitPromptTemplateByTokens } = proxyActivities<typeof prompt>({ startToCloseTimeout: '10 minute', });
 ...
 */
 export async function mysqlQuery< T >( dbhost: string, dbuser: string, dbpassword: string, dbname: string, sql: string, parameters: Array<any> ): Promise< Array<T> >
 {
 ...
 */
 export async function xNLPonDB(host: string, username: string, password: string, dbname: string, query: string): Promise< any >
 {
   ...
   let results: tableschema[] = await mysqlQuery( host, username, password, dbname, "SELECT TABLE_NAME, COLUMN_NAME FROM information_schema.columns WHERE TABLE_SCHEMA = ?", [dbname] );
   ...
   results.map( 
     (table_column) => { tables.set(table_column.TABLE_NAME, true) }
   );
   ...
   results.map(
     (table_column) => { tablesColumns.get(table_column.TABLE_NAME)!.push(table_column.COLUMN_NAME) }
   )
 ...
 For the natural language query "{{{query}}}", generate a comma-separated list of tables should we search, only from the above tables:`;
   ...
   prompt, {tablenames: tablenames, query: query }, 10, 50, 1.0 );
   ...
   r = "SELECT " + await promptTemplate(prompt, {query: query }, 10, 300, 1.0 );
