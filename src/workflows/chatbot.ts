...
export async function tooComplicatedChatbot(
  ...
  try {
    let rows = await es_query(
      `SELECT ts, user, text FROM messages ORDER BY ts DESC LIMIT ${context_length}`
    );
    ...
    ch.reverse();
    let r = await NaturalLanguageRoutine<ChatHistory[], any>(
      ch,
      async (x) => {
      ...
      '',
      (x) => {
        return JSON.parse('{ "bot_reply": "' + x);
      },
      `Complete ${personality.name}'s response to ${user} as a complete JSON document: with the following keys:\n\nbot_reply: Chuck's reply to U03UF3XNM8D\n\n{ "bot_reply": "`
    );
...
export async function xChatbot(
  personality: Personality,
  context_length: number,
  user: string,
  message: string,
  ...
  let p =
    ...
    (await promptTemplate(
      ...
      [ "Step 1:`,
      { message: message, fewshots: fewshots_joined, context: ch },
      10,
      500,
      0.0
    ));
 ...
 */
export async function NaturalLanguageRoutine<IN, OUT>(
  input: IN,
  input_decoder: decode_function<IN>,
  nl: string,
  out_schema: string,
  output_encoder: encode_function<OUT>,
  ...
  let p = await promptTemplate(
    t,
    {
      input: await input_decoder(input),
      nl: nl,
