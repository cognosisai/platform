...
const { es_index, es_drop } = proxyActivities<typeof elastic>({
  startToCloseTimeout: '10 minute'
});
const { nlp_stable_diffusion } = proxyActivities<typeof util>({
  startToCloseTimeout: '10 minute'
});
...
export async function storeEmbeddings(
  ...
  if (documents.length != sentences.length) {
    throw Error(
      'Error storing embeddings: sentences and documents are of different lengths'
    );
  }
