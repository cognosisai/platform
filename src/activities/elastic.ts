...
import { ElasticConfig } from '../config';
  ...
  export async function es_index( indexname: string, doc: any, refresh = true ): Promise< void >
  {
   ...
   */
  export async function es_search< T >( indexname: string, queryobj: any ): Promise< any >
  {
    let client = await getElasticSearchClient();
   ...
   */
  export async function es_context( indexname: string, path: string, line: number): Promise< any >
  {
    let client = await getElasticSearchClient();
    try
    {
  ...
  export async function es_mappings( index: string, doc: any, dims: number ): Promise< void >
  {
    let client = await getElasticSearchClient();
    /*
    ...
    */
    ...
    try
    {
      await client.indices.create({index: index, mappings: {properties: doc}} );
    }
    catch( e: any )
    {
...
{
  try { await es_drop( "messages" ); } catch (e: any) {}
  ...
  try { await es_drop( "teachings" ); } catch (e: any) {}
  ...
  try { await es_drop( "transcripts" ); } catch (e: any) {}
  ...
  try { await es_drop( "spider" ); } catch (e: any) {}
