...
import fs from 'fs';
import { getElasticSearchClient, es_context, es_delete, es_drop, es_index, es_mappings, es_query, es_search, init_elasticsearch_mappings } from './elastic';
 ...
 * @param {string[]} texts
 */
 export async function nlp_embeddings_internal( modelName: string, token: string, texts: string[] ): Promise< Map<string, number[]> >
 {
   try
   {
     ...
     {
       throw new Error(`Embeddings generation error: texts were ${texts.length} long, while there were ${obj.length} embeddings returned` );
     }
...
export async function embeddings_search( indexname: string, vector: number[], k: number ): Promise< any >
{
  console.log( `Searching for ${vector.length} in ${indexname} returning ${k}` );
  let client = await getElasticSearchClient();
  try
  { const result = await client.search( {index: indexname, body: {query: {script_score: {query: {match_all: {}}, script: {source: "cosineSimilarity(params.queryVector, 'embeddings') + 1.0", params: {queryVector: vector}}}}, size: k}} ); 
 ...
 */
 export async function nlp_embeddings( modelName: string, texts: string[] ): Promise< [string, number[]][] >
 {
   let token;
   var spawn = require('child_process').spawn; 
   process.env.GOOGLE_APPLICATION_CREDENTIALS='../deploy/production/etc/cognosis-hello-world-5b28cb5111ff.json'; // TODO: Fix me
   ...
   */
   ...
   {
     console.error( e.toString() );
     throw new Error(`Nu Embeddings generation error: ${e.toString()}\n${e.message}` );
   }
 }

 export async function embeddingsSearch( index: string, vector: number[], k: number ): Promise< any[] >
 {
   try
   {
