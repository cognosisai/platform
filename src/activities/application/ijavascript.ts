...
export const google = {
  search: async function (query: string): Promise<string[]> {
    ...
    let results = await executeWorkflow(
      TEMPORAL_HOST,
  ...
  get: async function (
    ...
    try {
      ...
      if (
        ...
        let pandoc = spawn('/usr/bin/pandoc', ['-f', 'html', '-t', 'plain'], {
...
export const nlp = {
  summarize: async function (text: string): Promise<string> {
    ...
    let r = await executeLocalWorkflow('mapreduce_summary', 'hello-world', [
      text
    ]);
    console.log(`Result: ${r}`);
    return <string>r;
  },
  question_answer: async function (
    text: string,
    question: string
  ): Promise<string> {
    console.log(`Summarizing: ${text}`);
    let r = await executeLocalWorkflow(
  ...
  ai_instruction_on_text: async function (
    text: string,
    instructions: string
  ): Promise<string> {
    console.log(`ai_instruction: ${text}`);
    let r = <any>(
      await executeLocalWorkflow('mapPromptTemplate', 'hello-world', [
        text,
        `{{{chunk}}}\n${instructions}:\n`
      ])
    );
...
async function executeJavascriptSnippetLoop(
  ...
  let r = await retryGenerateTextOpenAI(
    ...
    1,
    'text-davinci-002',
    ['Question:', 'Out [', error_in_stop, next_in_stop]
  );
  let origr = r;
  console.log(origr);
  ...
  if (origr.indexOf('Answer:') != -1) {
    ...
    try {
      ...
      console.log(
        `Strange. We got an answer but we couldn't get the last return value: ${e}`
      );
      return origr;
    }
  }
  ...
  try {
    ...
    if (processedOut.length > 512) {
      ...
      processedOut =
        processedOut.substring(0, 512) +
        `... Output truncated. Full output is in the 'return_${counter}' variable.`;
